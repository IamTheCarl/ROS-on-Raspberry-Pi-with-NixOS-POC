...
class Piper(Node):
    ...
    def speak(self, request, response):
        ...
        package_path = get_package_share_directory(request.voice_name)
        model_path = os.path.join(package_path, 'voice_models', '{}.onnx'.format(request.voice_name))
        self.get_logger().info("Model path: {}".format(model_path))
